...
class Style:
    ...
    def parse(s: str) -> Style:
        ...
        try:
            ...
            ret.name, ret.fontName, fontSize, pColor, sColor, oColor, bColor, bold, italic, underline, strikeout, scaleX, scaleY, spacing, angle, borderStyle, outline, shadow, alignment, marginL, marginR, marginV, encoding = rest.strip().split(',')
            ret.fontSize, ret.scaleX, ret.scaleY, ret.spacing, ret.marginL, ret.marginR, ret.marginV, ret.encoding = map(int, [fontSize, scaleX, scaleY, spacing, marginL, marginR, marginV, encoding])
            ret.angle, ret.outline, ret.shadow = map(float, [angle, outline, shadow])
            ret.primaryColor, ret.secondaryColor, ret.outlineColor, ret.backColor = map(Color.parse, [pColor, sColor, oColor, bColor])
            ret.isBold, ret.isItalic, ret.isUnderline, ret.isStrikeout = map(lambda x: x == '-1', [bold, italic, underline, strikeout])
